...
import csv
import math
path = r"/Users/yang/Downloads/前后车距离/data02/l2f2/车道4时间"  # 读取csv文件目录路径
path1 = r'/Users/yang/Downloads/前后车距离/data02/l2f2/data4'
...
for fn in FileNames:
    if re.search(r'\.csv$', fn):
        ...
        f = open(file,'r')
        reader = csv.reader(f)
        df = pd.read_csv(file,index_col='vehicle_id')          # 选择vehicle_id作为行索引
        # m用于存储每一行的元素
        m = []
        for row in reader:
            m.append(row)
